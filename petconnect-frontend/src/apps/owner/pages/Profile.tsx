...
import { MapPin, Mail, Phone, Edit, Camera, X, Save, Loader } from 'lucide-react'
  ...
  const fetchProfileData = async () => {
    try {
      ...
      const [profileRes, petsRes] = await Promise.all([
        ...
        api.get('/pets').catch(() => ({ data: { success: true, data: { pets: [] } } }))
      ])
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="text-center mb-6">
                <div className="relative inline-block mb-4">
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="absolute bottom-0 right-0 p-2 bg-purple-600 text-white rounded-full hover:bg-purple-700 transition-all shadow-lg disabled:opacity-50"
                  >
                    {uploading ? (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard title="My Pets" value={stats.pets.toString()} />
              <StatCard title="Total Walks" value={stats.walks.toString()} />
              <StatCard title="Total Spent" value={`KES ${(stats.spent / 1000).toFixed(1)}K`} />
              <StatCard title="Member Since" value={stats.memberSince.toString()} />
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                ...
                <InfoItem label="Last Updated" value={user?.updatedAt ? new Date(user.updatedAt).toLocaleDateString() : 'N/A'} />
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg font-medium hover:shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {saving ? (
                <>
